...
class DoctorViewModel : ViewModel() {
    ...
    private val _homeList: MutableLiveData<ArrayList<DoctorListModel>> = MutableLiveData()
    val _homeListData : LiveData<ArrayList<DoctorListModel>> get() = _homeList


    private fun get8Doctors() {
        viewModelScope.launch(Dispatchers.IO) {
            val db = Firebase.firestore
            val categoryCollection = db.collection("dentists").limit(5)
            categoryCollection.get()
                .addOnSuccessListener { documents ->
                    if (!documents.isEmpty) {
                        val list = ArrayList<DoctorListModel>()
                        ...
                        for (data in list){
                            Log.d("DOC LIST",data.certificate.toString())
                        }
                    } else {
                        println("Error getting documents: No Document Found")
                        val list = ArrayList<DoctorListModel>()
